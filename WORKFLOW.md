...
6. **CIG metaprops value-quality gate (CIG projects only, mandatory)**: Run the **`cig-monitoring-review`** skill (ideally as a subagent) as a Step 7 gate. It inspects metaprops **values** (not just key presence, which is gameable): it rejects placeholders, requires real citations and the `ServiceRequest.*` block on both the Goal and the Action, judges the case-report harm-not-reversibility rule, checks `monitored_target` traceability, and flags empty placeholder components and unpopulated Enquiry/Decision, emitting a readable pass/fail report under `projects/<project_dir>/reviews/`. Do not report the CIG as done while any FAIL remains.
