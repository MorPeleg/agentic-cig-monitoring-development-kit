...
**CIG monitoring:** the instantiation is an **ABox** that reuses the PROforma classes/properties from `cig/proforma.owl` by IRI (do not redeclare them). Add the PROforma prefix (`http://www.owl-ontologies.com/Ontology1779030093.owl#`). Create `Plan`/`Action`/`Decision`/`Enquiry`/`Component`/`Goal`/`Candidate`/`Schedule_Constraint` individuals connected via `components`, `taskref`, `goal`, `scheduled_constraints`, `conref`, `precondition`, and `candidates`. Express sequence with `Schedule_Constraint`/`conref` (PROforma has no "next" property) and parallelism by sharing a parent plan with no constraints. Assert `verb` (`achieve`/`avoid` for State Achievement goals, `order` for Action Enactment goals) and `noun_phrase` (referencing an HL7 FHIR `ServiceRequest` — `intent`, `code`, `occurrenceTiming`, `patientInstruction`, and `performerInstruction` for triggered labs — for monitoring orders) on each `Goal`. **Assert `metaprops` on every monitoring `Action`**: one `key=value; …` string that repeats the `ServiceRequest.*` order and documents provenance and operationalization (`source`, `knowledge_origin`, `guideline_status`, `literature_basis`, `assistant_invention`, `instrument`, `monitoring_frequency_basis`, `trigger`, `monitored_target`, `cq`) — so guideline-sourced facts are separable from self-designed ones and each action traces to the harm/goal it monitors (see `skills/cig-monitoring/references/ActionEnactmentGoal.md`). Imitate `cig/examples/obesity-glp1.owl`. When verifying in Step 7, pass `cig/proforma.owl` alongside the project ontology to `sparql_query` so the PROforma schema is available in the queried graph.
...
6. **CIG metaprops completeness (CIG projects only)**: Run the `metaprops` completeness `sparql_query` from the `cig-monitoring` skill to confirm every monitoring `Action` documents its ServiceRequest order, provenance/sources, instrument, frequency basis, and triggers. Any action the query returns is incomplete — fix it before reporting done.
